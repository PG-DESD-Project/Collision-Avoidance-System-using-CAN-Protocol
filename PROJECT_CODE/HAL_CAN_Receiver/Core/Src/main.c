...
void MotorStop(void);
void IR_Sensor_Receiver_Msg(void);
...
void IR_Sensor_Receiver_Msg(void) {
	...
	// Interpret IR sensor status: 1 = obstacle, 0 = clear
	if (RxData[2] == 1) {
		...
		snprintf(msg_buffer, sizeof(msg_buffer), "IR Sensor: Area clear\r\n");
	}

	// Transmit the status over UART
