...
{
    // Assuming SystemCoreClock is set correctly
    uint32_t start = DWT->CYCCNT;
    uint32_t count = (SystemCoreClock / 1000000) * microseconds;
...
void Read_Ultrasonic(void)
{
    ...
    while (!isReadingFinished)
        ; // This blocks the loop until echo is received. Wait for measurement to complete
    isReadingFinished = 0;
}
/* USER CODE END 0 */
